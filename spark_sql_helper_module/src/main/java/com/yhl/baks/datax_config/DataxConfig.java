package com.yhl.baks.datax_config;
 ...
 */
public class DataxConfig implements Serializable{
	 ...
	 * 初始化空白的Configuration
	 */
	public static DataxConfig newDefault() {
		return DataxConfig.from("{}");
	}

	/**
	 * 从JSON字符串加载Configuration
	 */
	public static DataxConfig from(String json) {
        json = StrUtil.replaceVariable(json);
		checkJSON(json);

		try {
			return new DataxConfig(json);
		} catch (Exception e) {
	 ...
	 */
	public static DataxConfig from(File file) {
		try {
			return DataxConfig.from(IOUtils
	 ...
	 */
	public static DataxConfig from(InputStream is) {
		try {
			return DataxConfig.from(IOUtils.toString(is));
		} catch (IOException e) {
			throw DataXException.asDataXException(CommonErrorCode.CONFIG_ERROR,
	 ...
	 */
	public static DataxConfig from(final Map<String, Object> object) {
		return DataxConfig.from(DataxConfig.toJSONString(object));
	}

	/**
	 * 从List对象加载Configuration
	 */
	public static DataxConfig from(final List<Object> object) {
		return DataxConfig.from(DataxConfig.toJSONString(object));
	}
	 ...
	 */
	public DataxConfig getConfiguration(final String path) {
	 ...
	 */
	public List<DataxConfig> getListConfiguration(final String path) {
		List<Object> lists = getList(path);
		if (lists == null) {
			return null;
		}

		List<DataxConfig> result = new ArrayList<DataxConfig>();
		for (final Object object : lists) {
			result.add(DataxConfig.from(DataxConfig.toJSONString(object)));
		}
		return result;
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public Map<String, DataxConfig> getMapConfiguration(final String path) {
		Map<String, Object> map = this.get(path, Map.class);
		if (null == map) {
			return null;
		}

		Map<String, DataxConfig> result = new HashMap<String, DataxConfig>();
		for (final String key : map.keySet()) {
			result.put(key, DataxConfig.from(DataxConfig.toJSONString(map
					.get(key))));
		}
	 ...
	 * @return 返回合并后对象
	 */
	public DataxConfig merge(final DataxConfig another,
	...
	public String toJSON() {
		return DataxConfig.toJSONString(this.getInternal());
	}

	/**
	 * 拷贝当前Configuration，注意，这里使用了深拷贝，避免冲突
	 */
	public DataxConfig clone() {
		DataxConfig config = DataxConfig
				.from(DataxConfig.toJSONString(this.getInternal()));
	...
	private Object extractConfiguration(final Object object) {
		if (object instanceof DataxConfig) {
	...
	private Object extractFromConfiguration(final Object object) {
		if (object instanceof DataxConfig) {
			return ((DataxConfig) object).getInternal();
		}
	...
	private DataxConfig(final String json) {
		try {
